...
public:
    int helper(int i, int amt, vector<int> &coins, vector<vector<int>>& dp){
        if(amt==0) return 1; //Found 1 valid array
        if(i==coins.size() || amt<0) return 0; // Out of bounds or invalid
        if(dp[i][amt]!=-1) return dp[i][amt];
        // Take the current coin
        int take=helper(i, amt-coins[i], coins, dp);
        //Skip the current coin
        int not_take=helper(i+1, amt, coins, dp);
        return dp[i][amt]=take+not_take;
    }
